 ...
 *   →  the machine he builds  →  the name of each place he has worked
 *
...
export async function createStoryScene(
  ...
  /**
   * Every name the dots can take, sampled once and kept. Switching between them
   * is then a copy rather than a fresh sampling, so stepping through the places
   * he has worked costs nothing while the page is scrolling.
   *
   * Sampled at one device pixel per point rather than two: five full-view masks
   * at retina density is a visible stall on resize, and the dots do not need
   * that precision.
   */
  const layoutWords = () => {
